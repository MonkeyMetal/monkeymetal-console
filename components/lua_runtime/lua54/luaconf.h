...
/* ── ESP32-S3 overrides ────────────────────────────────────────────────────
** Applied before the defaults below so the #if !defined guards pick them up.
** Single-precision float (ESP32-S3 has hardware FPU for float, not double).
** 32-bit integer (native word size, saves PSRAM vs 64-bit default).
** Reduced stack depth to stay within 2 MB Lua heap budget.
