...
// SD card headers removed — MonkeyMetal manages its own SD mount via port_bsp
// Stub types to satisfy codec_init.c references without pulling in fatfs
typedef void* sdmmc_card_t_stub;
#define sdmmc_card_t sdmmc_card_t_stub
...
// SD card functions stubbed out — MonkeyMetal manages SD via port_bsp/sdcard_bsp
// codec_board's SD mount is not used in this project.

int mount_sdcard(void)   { return -1; }
void *get_sdcard_handle(void) { return NULL; }
